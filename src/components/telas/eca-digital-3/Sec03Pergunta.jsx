...
// TELA PRÓPRIA da pergunta da seção 3 do Álbum 3 (Artigo 28). Layout (definido
// pelo usuário): balão LARANJA/texto branco no topo (bico pra baixo); personagem
// no tamanho máximo; e duas opções em BOLA branca (borda + letra azul-escuro)
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col items-center justify-center p-4">
        {/* balão laranja com bico apontando pra baixo (pra personagem, abaixo). */}
        <div
        ...
        <div className="relative mt-4 flex w-full items-end justify-center">
          {personagemSrc && <Personagem src={personagemSrc} className="w-full" />}
          <div className="absolute bottom-40 left-0 w-7/15">{opcao(esquerda)}</div>
          <div className="absolute bottom-40 right-0 w-7/15">{opcao(direita)}</div>
        </div>
      </div>
    </section>
  );
